...
const USE_CASES = [
  { id: 'coverage-eligibility', icon: Shield,       title: 'Coverage Eligibility',  description: 'Check patient insurance coverage and benefits in real-time via NHCX', color: '#34d399', border: 'rgba(52,211,153,0.3)',  bg: 'rgba(52,211,153,0.08)',  tag: 'CoverageEligibilityRequest' },
  { id: 'claim',                icon: FileText,      title: 'Claim Submission',       description: 'Convert and submit institutional claims conforming to NHCX FHIR profiles', color: '#10b981', border: 'rgba(16,185,129,0.3)', bg: 'rgba(16,185,129,0.08)', tag: 'Claim' },
  ...
  { id: 'communication',        icon: MessageSquare, title: 'Communication',          description: 'Bundle medical records, discharge summaries and documents for NHCX', color: '#2dd4bf', border: 'rgba(45,212,191,0.3)',   bg: 'rgba(45,212,191,0.08)',  tag: 'Communication' },
];
...
const INPUT_FORMATS = [
  ...
  { icon: Wand2,           label: 'Free Text',   desc: 'LLM-powered extraction' },
];

const STATS = [
  { label: 'FHIR Resources', value: '12+', color: '#34d399' },
  { label: 'SNOMED Codes',   value: '50+', color: '#10b981' },
  { label: 'Input Formats',  value: '6',   color: '#2dd4bf' },
  { label: 'NHCX Profiles',  value: '8',   color: '#6ee7b7' },
];
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-10">
      ...
      <motion.div variants={stagger} initial="initial" animate="animate">
        ...
        <motion.h1 variants={fadeUp} className="font-display text-5xl font-bold text-white leading-tight mb-4">
          UniHealth —{' '}
          <span className="gradient-text">Legacy to FHIR</span>
        </motion.h1>
        <motion.p variants={fadeUp} className="text-emerald-100/70 text-lg max-w-2xl font-body leading-relaxed">
          Transform HL7 v2.x, CSV, XML, and HMIS data into NHCX-aligned FHIR bundles
          with AI-powered schema mapping and automatic SNOMED CT coding.
        </motion.p>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        {STATS.map(s => (
          <div key={s.label} className="card text-center" style={{ borderColor: s.color + '40' }}>
            <div className="font-display text-4xl font-bold mb-2" style={{ color: s.color, textShadow: `0 0 20px ${s.color}60` }}>{s.value}</div>
            <div className="text-emerald-200/80 text-sm font-semibold">{s.label}</div>
          </div>
        ))}
      </motion.div>
              ...
              <motion.div key={uc.id}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-emerald-100/70 text-sm leading-relaxed">{uc.description}</p>
                    <div className="mt-3">
        ...
        <div className="grid grid-cols-4 gap-3">
          {INPUT_FORMATS.map(({ icon: Icon, label, desc }, i) => (
            <motion.div key={label}
              ...
              <div className="text-emerald-200/60 text-xs">{desc}</div>
            </motion.div>
          ))}
        </div>
      </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="card">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="ml-auto flex items-center gap-2">
              <div className="w-2.5 h-2.5 rounded-full bg-emerald-400" style={{ boxShadow: '0 0 10px #34d399' }} />
          ...
          <div className="grid grid-cols-3 gap-4">
            {Object.entries(health.llmProviders || {}).map(([provider, active]) => (
              <div key={provider} className="flex items-center gap-3 p-4 rounded-xl"
                style={{ background: '#071a12', border: `1px solid ${active ? 'rgba(16,185,129,0.35)' : '#1a4a30'}` }}>
                <Zap className={`w-5 h-5 ${active ? 'text-emerald-400' : 'text-emerald-900'}`} />
                <div>
                  <div className="text-white text-sm font-bold capitalize">{provider}</div>
                  <div className={`text-xs font-semibold ${active ? 'text-emerald-400' : 'text-emerald-900'}`}>
